...
public abstract class AnnotationConfigUtils {
	...
	public static Set<BeanDefinitionHolder> registerAnnotationConfigProcessors(
		 ...
		 *
		 * ConfigurationClassPostProcessor是一个工厂后置处理器，这个后置处理器非常重要，
		 * 基本上类上面的注解都在这里面判断并解析，spring的包扫描也在里面完成
		 */
		 ...
		 * 提示： 这里有一个jsr250Present校验 就是判断@Resource的路径存不存在
		 */
	...
	private static BeanDefinitionHolder registerPostProcessor(
		...
		/**
		 * AopTest类中new AnnotationConfigApplicationContext
		 *
	 ...
	 * 并添加到数据结构中去
	 *
	 * 主要处理Lazy、DependOn、 Primary 、Role等注解，然后将注解的信息添加到数据结构BeanDefinition中去，
	 * BeanDefinition就是描述bean注册的信息
	 */
